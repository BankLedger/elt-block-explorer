...
Bitcoin.DEFAULT_CONFIG_SETTINGS = {
  ...
  timestampindex: 1,
  spentindex: 1,
  zmqpubrawtx: 'tcp://127.0.0.1:11109',
  zmqpubhashblock: 'tcp://127.0.0.1:11109',
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getMyInfo', this, this.getMyInfo, 0],
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  var configPath = path.resolve(spawnOptions.datadir, './eltoken.conf');
...
Bitcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  ...
  var paginate ={
	  'txs':{
		  $slice :[parseInt(fromArg),11]
	  }
  }
  db.address.find({'address':addressArg},paginate).toArray(function(err,addressList){
    if (err) {
      return callback(err);
    }
	var txids = [];
    ...
    if(addressList && addressList[0]){
    	txids = addressList[0].txs;
    	if(txids.length === 11){
        	totalCount = parseInt(fromArg) + txids.length;
        	txids.pop();
        }else{
        	totalCount = parseInt(fromArg) + txids.length;
        }
 ...
 * @param {Function} callback
 */
Bitcoin.prototype.getAddressSummaryOld = function(addressArg, options, callback) {
	var self = this; 
	var summary = {};
	var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
	var summaryTxids = [];
    ...
    async.parallel([
      function getTxList(done) {
        ...
        self.getAddressTxids(addresses, {queryMempool: false}, function(err, txids) {
          if (err) {
        	    console.log("getAddressTxids:"+err.message);
            return done(err);
          }
          summaryTxids = txids;
      ...
      function getBalance(done) {
        self.getAddressBalance(addresses, options, function(err, data) {
          if (err) {
        	  console.log("getAddressBalance:"+err.message);
            return done(err);
          }
      ...
      function getMempool(done) {
        ...
        self.client.getAddressMempool({'addresses': addresses}, function(err, response) {
          if (err) {
        	  console.log("getAddressMempool:"+err.message);
...
Bitcoin.prototype.getAddressSummary = function(address, options, callback) {
	...
	db.address.find({'address':address}).toArray(function(err,addressObj){
		...
		if(addressObj && addressObj[0]){
			...
			console.log("getAddressSummary no data found in db ");
			callback(null,null);
		}
	})
};
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          try{
        	  ...
        	  console.log("not exist block............hash is:"+blockhash);
        	  client.getBlock(blockhash, true, function(err, response) {
        		  try{
        			  ...
        			  console.log("not exist block............hash:"+blockhash+" try to get with true and parse angain....but still not work.....try agin.....with height");
        			  var height=response.result.height;
        			  client.getBlockByNumber(height, function(err, response) {
        				  console.log("now we get it is height:"+height);
        				  var rightHash=response.result.hash;
        				  console.log("now we get it is right hash:"+rightHash);
        				  client.getBlock(rightHash, false, function(err, response) {
        					  ...
        					  try{
        						  ...
        						  console.log("we can not get the right block even by it is right hash!!!!!!:"+rightHash);
        					  }
        				  });
        			  });
        		  }
...
Bitcoin.prototype.getDetailedTransaction = function(txid,callback) {
  ...
  function isLocked(unlockHeight,currBlockMaxHeight,txBlockHeight) {	
		if(unlockHeight && unlockHeight>0){
			if(unlockHeight > currBlockMaxHeight){
				return true;
			}
		}
		return false;
      ...
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        self.getInfo(function(err,info){
          if(err){
      	  ...
      	  if(!result.blockhash){
      		  log.error("new Transaction from ZMQ txid:"+txid+" has no block hash yet default is block height +1");
...
Bitcoin.prototype.getMyInfo = function(callback) {
	  var self = this;
	  this.client.getInfo(function(err, response) {
	    if (err) {
	      return callback(self._wrapRPCError(err));
