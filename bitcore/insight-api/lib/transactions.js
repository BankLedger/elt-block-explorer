...
TxController.prototype.list = function(req, res) {
  ...
  getTrsansationByAddress(self,address, options,function(list){
    	if(list){
    		res.jsonp(list);
    	}
    });
...
function getTrsansationByAddress(self,address, options,callback){
	self.node.getAddressHistory(address, options, function(err, result) {
	      ...
	      async.map(
	        ...
	        function(err, transformed) {
	          ...
	          if(transformed){
	        	  callback({
	                  pagesTotal: Math.ceil(result.totalCount / 10),
	                  txs: transformed
	                });
	          }else{
	        	  options = {
	        	  ...
	        	  getTrsansationByAddress(address, options,callback);
	          }
	        }
	      );
	    });
